---
title: "Analysis with Raster Data Using R and rasterList Package: saving functions ot any kind of data in a grid cell"
...
object for each raster cells. In such a way,  different kinds of objects, e.g. the probability distrubution of the mapped random variable or an _htest_, _lm_ ,_glm_ or _function_ class objects (@Hastie1992a,@Hastie1992b), can be saved for each cell, in order that the user can save intermediate results without loosing any  geographic information. This new oblect class  is implemented in a new packge __RasterList__. Two  applications are  presented: 

 * Creation of a 2D/3D modelled map of  soil water retention curve based on existing soil properties map and state-of-art empirical formulation (e.g. @VanGenuchten1980);
 * Processing spatial gridded datasets of daily or momthly precipitation (e. g. @chirps2015) for a trend or frequancy analysis (estimation of the return period of critical events).


## Application 1 

The application 1 is the estimation of the mathematical reletionships between soil water content $\theta$ and soil water pressure $\psi$, i.e. soil water retention curve. Soil water retention curve is widely used in subsurface water/groundwater hydrological modeling because it allows to close the water balance equation. Generally some theoretical formulas are used to model this curve in function of soil properties. In this examples, given a raster map  of soil properties, a map of soil water retention curves is produced.
...
```
It it is assumed that each cell of a raster maps has its own soil water retention curve. Therefore to map the soil water retion curve and not only its parametes, __RasterList__ package is loaded: 
```{r eval=TRUE}
library(rasterList)
```
...
```
A geografic view of Meuse test case is available though __leaflet__ package (@leaflet): 
```{r eval=TRUE}
library(rasterList)
library(soilwater)
library(leaflet)
...
leaf2
```
Therefore, a map of each @VanGenuchten1980's formula parameter can be obtained as follow (The names of saturated and residual water contents _swc_ and _rwc_ mean $\theta_{sat}$ and $theta_{res}$ according to the the arguments of _swc_  function of __soilwater__ package (@soilwater))
```{r eval=TRUE}
...
```
Let consider 3 point of interest in the Meuse located (latitude and logitude) as follows:
```{r eval=TRUE}
...
```
Using _swc_ , a function that given the soil parameters returns a soil water function is defined as follows:
```{r eval=TRUE}
library(soilwater)
...
```
Finally, assuming that $\psi$ may vary with space ranging between -0.5 m and -1.5 m with a spatial parabolid-like profile centred in point A, a map for $\psi$ is calculated as follows:
```{r eval=TRUE,warings=TRUE}
...
```
Total annual precipitation can be computed through an aggregation of monthly data with a _sum_ function aggragtion and _lubridate_ package (@lubridate).
```{r eval=TRUE,warnig=FALSE,message=FALSE}
...
```
Mann-Kendall trend analysis (@trend) is performed in  order to detect if annual precipitation is increasing or decreasing with time. Firstly the attention is focused on four check points in the region of interest going from the Andea higlands to the Amoazon Rainforest: Cajamarca city, Imaza (Amozanas) and Angoretos and Iquitos (Loreto). 
```{r eval=TRUE}
...
lsl
```
From test results, in Cajamarca and Angoteros mean annual precipitation has no significant trend (i.e. p-value is greater that the accepted eual or lower thenthe significance of  0.05) whereas a trend results in Imaza and Iquitos. A graphical representation of mean annaual precipitation with trend and a web map of trend in all area are available below:
```{r eval=TRUE}
...
ptstrend <- list()
 ...
 for (i in 1:nrow(pts)){
  ...
  do.call(what=grid.arrange,args=ptstrend) 
```
```{r eval=TRUE,warning=FALSE,ercho=FALSE,message=FALSE}
...
The behaviour of mean monthly precipitation within the rainy season is similar to the behaviour of the total annual precipitation. Trends, where significantly exist, are not very high. However, mean monthly precipitation within the rainy season an indicator of rain water availablity for agriculture during the growing season.  In the following, both annual mean precipitation and mean monthly precipitation within the rainy season are rescaled and carried out homogenized with the reference to the conditional expected value of the last year of the time series, i.e. 2017, in order that the time series values are assumed to be independent and identically distributed:
```{r eval=TRUE}
...
```{r eval=TRUE}
  ...
  ks
```
The procedure  is therefore replied at a raster scale (for each pixel of the raster) by defining and creating the following structures:
== _samlmom_ a _RasterStack-class_ Object containing L-Momnts for each pixel;
...
These objects are repliceted for everery hypothetical probability distrubition.
```{r eval=TRUE,warning=FALSE}
...
```
THe mapping of _p.value_ is useful to assess where the null hypothesis (e.g. the probability distrubution) can be accepted or not. Both selected probability distributions can be accepted for all raster cells.
```{r eval=TRUE}
  pval_ks <- list()
...
```
Return period is  the expected time interval between two events where precipitation is lower than a certain threshold value. Return periods are a measure of the frequency at which specific events occurs. Since the object is to analyze the meteorological drought (i.e. lack of rainfall), return periods are the inverse of quantiles associeted to a precipitation lower that the mapped value. Under the hypothesis that the probability distribution is Pearson III type (i.e. _pe3_), return periods are calculated as follows:
...
```
Finally, here is a visualization of the value of mean monthly precitation in the 3 most rainy months of the year that is not reached with a return period of 20 years:
```{r eval=TRUE,warning=FALSE,ercho=FALSE,message=FALSE}
r <-  percentiles[["T20"]]
rp <- r
r[r>1000] <- 1000
...
The above applications illustrate how statistical analysis and other processing may be replicated at every cell of a raster map. The __rasterList__ package has been created to answer the need to make complex operation on spatially gridded region. The shown examples go beyond their own task but they have the task of teaching the most important functions of the package and therefore how to make use of the package. 
...
This is of great relavance because it allows to save intermediate steps during a processing workflow (e.g. statistical analysis).
